...
Use `index.ts` only at layer roots (`screens`, `widgets`, `features`,
`entities`, `shared`) to define the public API.

Do not create slice-level or segment-level public APIs via `index.ts`.

```typescript
// features/index.ts
export { LoginForm } from "./auth/ui/LoginForm";
export { AddToCartButton } from "./add-to-cart/ui/AddToCartButton";
```
