...
screens/   → Screen-level composition, owns its own logic
...
### 4-2. Public API: re-export only at layer root

External consumers may only import from the layer root `index.ts` files:
`screens/index.ts`, `widgets/index.ts`, `features/index.ts`,
`entities/index.ts`, and `shared/index.ts`.

Do not create or rely on `index.ts` public APIs inside slices or shared
segments. Direct imports of internal files are forbidden.
...
// ❌ Violation: slice-level public API
import { LoginForm } from "@/features/auth";
```

**Shared layer:** Keep shared exports in the top-level `shared/index.ts`
instead of segment-level public APIs (`shared/ui/index.ts`,
`shared/api/index.ts`, etc.).

### Environment-specific public APIs

A layer should normally expose its public API through a single root
`index.ts`. Ad-hoc customization is not recommended.

If a single layer entry point cannot preserve a runtime boundary, add an
layer-level environment-specific entry point such as `index.server.ts`. See
...
- **Strategy D: Public API access.** When reuse is genuinely unavoidable,
  allow it only through the layer root `index.ts`. Never reach into `model/`,
...
- **Public API level**: Re-export only at layer roots (`screens`, `widgets`,
  `features`, `entities`, `shared`), not inside slices/segments.
...
- **When integrating FSD with a specific framework** (Next.js with App Router
  or Pages Router, Nuxt, Vite, CRA, Astro) for wiring routes to FSD screens,
